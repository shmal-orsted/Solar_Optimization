...
def api_call_func(api_key, api_url, src_args, weather_df, gcr_range, dc_range):
    """
    ...
    for x in range(gcr_range[0], gcr_range[1], 5):
        for y in range(dc_range[0], dc_range[1], 10000):

            # Change the src_args values to the current range value
            src_args["DCCapacity"] = y
            src_args["GCR"] = x/100
            ...
            if (src_energy_manual_request.status_code == 200):
                ...
                outputs_dict[f"{y} DC, {x} GCR"] = json_dict
                outputs_list.append(json_dict)

    return outputs_dict, outputs_list
